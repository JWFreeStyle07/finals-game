...
#
## How much each stat drains per second passively
#const HEALTH_DRAIN    := 0.5
...
##  MOVEMENT
## ─────────────────────────────────────────────
#const MOVE_SPEED      := 80.0   # pixels/sec — tweak for your tile size
#const IDLE_THRESHOLD  := 10.0   # velocity below this = idle animation
#
## ─────────────────────────────────────────────
##  INVENTORY
## ─────────────────────────────────────────────
...
#var held_item: Dictionary = {}     # Item currently "held" / shown above player
#var active_slot: int = 0           # Which inventory slot is selected (0-based)
#
...
##  INTERACTION
## ─────────────────────────────────────────────
#var nearby_interactable: Node2D = null   # Set by InteractionArea signals
#
...
#
#var last_direction := Vector2.DOWN  # used for idle/facing animation
#
#
## ═════════════════════════════════════════════
##  LIFECYCLE
...
#
#func _ready() -> void:
	## Connect Area2D signals for nearby interactables
...
#
	## Initialise inventory slots as empty
...
##  CAMERA
## ═════════════════════════════════════════════
## The Camera2D is a child of Player so it follows automatically.
## Call set_camera_limits() from your Level script once the TileMap loads,
## passing in the bounding rect of your room/map so the camera won't
## show black borders outside the level.
##
## Example from your Level.gd:
##   var bounds = $TileMap.get_used_rect()
##   var tile_size = $TileMap.tile_set.tile_size
##   player.set_camera_limits(bounds, tile_size)
#
#func _setup_camera() -> void:
	#camera.make_current()   # ensure this camera is the active one
#
...
#func set_camera_zoom(zoom_level: float) -> void:
	## Handy if you want different zoom per level (e.g. bigger rooms zoom out)
...
#func _get_input_direction() -> Vector2:
	## Works with keyboard AND a virtual joystick (set up an InputMap action for each)
...
#
## ═════════════════════════════════════════════
##  INPUT — Interactions, inventory hotkeys
## ═════════════════════════════════════════════
#
...
#
	## If we're holding nothing and the interactable is a pickable item
	#if held_item.is_empty() and nearby_interactable.has_method("get_item_data"):
		#_pickup(nearby_interactable)
#
	## If we ARE holding something and the target can receive it
...
#func _pickup(item_node: Node) -> void:
		...
		## TODO: play "no space" sound / show popup
		#return
#
...
#
	...
	#held_item = item_data
#
	## Add to active slot, or first empty slot
...
#
	## Tell the item node it's been picked up (it can hide itself)
...
#
	## Tell the target it's receiving an item
...
#func _set_active_slot(index: int) -> void:
	...
	## Update held_item to match slot
...
#func _drain_stats(delta: float) -> void:
	#_modify_health(-HEALTH_DRAIN * delta)
	#_modify_energy(-ENERGY_DRAIN * delta)
...
#
#
## Call this from items/events that boost stats (e.g. task completed)
#func restore_stats(h: float = 0, e: float = 0, hap: float = 0) -> void:
	#_modify_health(h)
	#_modify_energy(e)
	#_modify_happiness(hap)
#
#
#func _check_death() -> void:
		...
		## Let the level/game manager know
...
##  ANIMATIONS
## ═════════════════════════════════════════════
## Expects your spritesheet to have these animation names set up
## in AnimatedSprite2D > SpriteFrames:
...
#func _play_walk_animation(dir: Vector2) -> void:
	#var anim := _direction_to_anim_suffix(dir)
	#var anim_name := "walk_" + anim
...
#func _play_idle_animation(dir: Vector2) -> void:
	#var anim := _direction_to_anim_suffix(dir)
	#var anim_name := "idle_" + anim
	#if animated_sprite.animation != anim_name:
		#animated_sprite.play(anim_name)
#
#
#func _direction_to_anim_suffix(dir: Vector2) -> String:
	## Prioritise vertical for diagonal movement (common in top-down games)
